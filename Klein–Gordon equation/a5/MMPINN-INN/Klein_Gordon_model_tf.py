...
class Klein_Gordon:
    # Initialize the class
    def __init__(self, layers, operator, ics_sampler, bcs_sampler, res_sampler,u, alpha, beta, gamma, k, model, stiff_ratio):
        ...
        self.W_t =tf.Variable(tf.random_normal([1, layers[0] //2], dtype=tf.float32)  * 5, dtype=tf.float32, trainable=False)
        ...
        # Total loss
        self.loss = tf.pow(self.loss_res,1/3) +tf.pow( self.loss_u,1/3)
        ...
        t, x = np.meshgrid(t, x)
        self.X_star = np.hstack((t.flatten()[:, None], x.flatten()[:, None]))
        self.u_star = u(self.X_star)       
        self.l2_error_log = []
