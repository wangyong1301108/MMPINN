...
class PhysicsInformedNN:
    ...
    def __init__(self, x0, u0, tb, X_f, layers, lb, ub,u_lb,u_ub):
        ...
        self.encoder_weights_1 = self.xavier_init([ layers[1], layers[1]])
        self.encoder_biases_1 = self.xavier_init([1, layers[1]])

        self.encoder_weights_2 = self.xavier_init([ layers[1], layers[1]])
    ...
    def neural_net(self, X, weights, biases):
        ...
        encoder_11 = tf.tanh(tf.add(tf.matmul(H, self.encoder_weights_1), self.encoder_biases_1))
        encoder_21 = tf.tanh(tf.add(tf.matmul(H, self.encoder_weights_2), self.encoder_biases_2)) 
        ...
        for l in range(0,num_layers-2):
            W = weights[l]
            b = biases[l]
            H = tf.multiply(tf.tanh(tf.add(tf.matmul(H, W), b)), encoder_11) +                     tf.multiply(1 - tf.tanh(tf.add(tf.matmul(H, W), b)), encoder_21)
    ...
    def net_f_uv(self, x, t):
        ...
        '''
        f_u = u_t-u_xx-(20*math.pi*tf.cos(20*math.pi*t/(9*x**2 + 1))/(9*x**2 + 1))+(-129600*math.pi**2*t**2*x**2*tf.sin(20*math.pi*t/(9*x**2 + 1))/(9*x**2 + 1)**4 + 12960*math.pi*t*x**2*tf.cos(20*math.pi*t/(9*x**2 + 1))/(9*x**2 + 1)**3 - 360*math.pi*t*tf.cos(20*math.pi*t/(9*x**2 + 1))/(9*x**2 + 1)**2)
        return f_u    
...
def heatsolution(x,t):
    return math.sin(20*np.pi*t/(1+9*x**2))
